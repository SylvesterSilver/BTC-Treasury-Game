...
function getRating(btcValueMM: number, daysSurvived: number): { title: string; color: string; desc: string } {
  ...
  if (b >= 10)         return { title: 'DIAMOND HANDS',       color: '#22c55e', desc: 'You navigated volatility like a pro.' };
  if (b >= 5)          return { title: 'STRONG HOLD',         color: '#60a5fa', desc: 'Solid treasury operations. Room to grow further.' };
...
function LeaderboardTable({ entries, highlightScore }: { entries: LeaderboardEntry[]; highlightScore?: number }) {
  if (entries.length === 0) {
    return <div className="text-[#2a3a52] text-xs text-center py-3">No scores yet — you'll be first!</div>;
  }
  return (
      ...
      {entries.map((e, i) => {
        ...
        return (
          <div key={i}
            ...
            {/* Date */}
            <span className="text-[#2a3a52] hidden sm:block">{e.date}</span>
          </div>
        );
      })}
    </div>
...
export function GameOverScreen({ isWin, metrics, balance, era, daysSurvived, onRestart, onChangeEra }: Props) {
  ...
  return (
    <div className="modal-overlay">
      <div
        className="bg-[#0a0f1e] border border-[#1a2540] rounded-xl mx-4 relative overflow-y-auto"
        ...
        <div className="p-5">
          ...
          <div className="text-center mb-4">
            {isWin ? (
              <>
                ...
                <div className="text-[#3a5070] text-xs mt-1">Bitcoin treasury mission accomplished</div>
              </>
            ) : (
              <>
                <div className="text-5xl mb-1">📉</div>
                <div className="text-red-400 text-2xl font-bold">REKT</div>
                <div className="text-[#3a5070] text-xs mt-1">{metrics.insolventReason ?? 'The preferred dividends ate you alive.'}</div>
              </>
            )}
          </div>
          ...
          {qualifies && !submitted && (
            <div className="rounded-lg border border-bitcoin p-4 mb-4 glow-bitcoin"
              ...
              <div className="flex gap-2">
                <input
                  className="flex-1 bg-[#060a12] border border-bitcoin/50 rounded px-3 py-2 text-sm font-mono text-white focus:outline-none focus:border-bitcoin"
          ...
          <div className="game-card p-3 mb-4">
            <div className="section-label mb-2 flex items-center gap-2">
              ...
              <span className="text-[#2a3a52] text-xs ml-auto">by stock price</span>
            </div>
          ...
          <div className="rounded-lg border border-[#F7931A33] p-3 mb-4"
            style={{ background: 'linear-gradient(135deg, #0a1408, #060a12)' }}>
            <div className="section-label mb-2 text-bitcoin/60">₿ BITCOIN PRICE JOURNEY</div>
            <div className="grid grid-cols-3 gap-2 items-center">
              <div>
                <div className="text-[#3a5070] text-xs mb-0.5">Start</div>
              ...
              <div className="text-center">
                ...
                <div className="text-[#2a3a52] text-xs">{daysSurvived} days</div>
              </div>
              <div className="text-right">
                <div className="text-[#3a5070] text-xs mb-0.5">End</div>
          ...
          <div className="game-card p-3 mb-4">
            ...
            <StatRow label="BTC Treasury Value" value={fmtMM(metrics.btcValueMM)} color={metrics.btcValueMM > 1000 ? '#22c55e' : '#e2e8f0'} />
            <StatRow label="mNAV at End" value={`${metrics.mNAV.toFixed(2)}x`} />
            <StatRow label="Cash Remaining" value={fmtMM(balance.cashMM)} color={balance.cashMM > 0 ? '#22c55e' : '#ef4444'} />
          </div>
